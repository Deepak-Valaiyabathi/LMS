...
const LeaveForm: React.FC<LeaveFormProps> = ({ leaveBalance }) => {
  ...
  const historyDate = async () => {
    try {
      ...
      const response = await fetch("http://localhost:5000/api/leave-date-history", {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
      ...
      <form onSubmit={handleSubmit} className="h-[48vh] my-5 place-items-center flex flex-col justify-around text-[17px]">
        ...
        <div className="px-7 flex flex-col">
          ...
          <select
            ...
            className={`w-[300px] border-2 py-2 px-2 rounded-lg focus:outline-none ${ errors.leaveId && touched.leaveId ? 'border-red-500' : 'border-gray-300'}`}
            value={values.leaveId}
            // onChange={(e) => setFormData((prev) => ({ ...prev, leaveId: Number(e.target.value) }))}
            onChange={handleChange}
            onBlur={handleBlur}
            >
